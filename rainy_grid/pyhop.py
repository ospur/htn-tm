...
def pyhopT(state, tasks, critic, verbose=0):
    """
    ...
    result = seek_plan(state, tasks, [], 0, critic, verbose)

    if verbose>0: print('** result =', result, '\n')
    return result

def seek_plan(state, tasks, plan, depth, critic, verbose=0):
    """
    ...
    if tasks == []:
        ...
        #return plan
        return plan #added returning of state

    task1 = tasks[0]
    if task1[0] in operators:
        if verbose>2: print('depth {} action {}'.format(depth,task1))
        operator = operators[task1[0]]
        newstate = operator(state, *task1[1:])
        # Begin my code
        # the idea is that after we commit to an operator, we dyamically update the
        # domain
        # move_ops = {'move_up', 'move_forward', 'move_down','move_backward'}
        # if newstate:
        #     if task1[0] in move_ops:
        #         rain(newstate, task1[1])
        #         #increment number of tiles seen
        #         newstate.prob['m'] += 1
        #         newstate.prob_cur['m_cur'] += 1
            #elif task1[0] == '
        ...
        if newstate:
            tasks = critic.ruminate(newstate, tasks[1:])
            # print("\noperator", tasks)
            # print("agent loc", state.agent)
            solution = seek_plan(newstate,tasks,plan+[task1],depth+1, critic, verbose)
            if solution != False:
                return solution
    if task1[0] in methods:
        ...
        for method in relevant:
            ...
            if subtasks != False:
                solution = seek_plan(state,subtasks+tasks[1:],plan,depth+1, critic, verbose)
                # print("\nmethod", subtasks+tasks[1:])
                # print("agent loc", state.agent)
                if solution != False:
                    return solution
...
############################################################
# Dynamic updating functions. My code.

def rain(state, agent):
    loc=state.agent[agent]
    x = randint(1,100)
    #beacon S1 gives a 99% chance of rain
    if state.lit_test["S1"] == 1:
        if (x <= 99):
            state.clear[loc] = 0
        else:
            state.clear[loc] = 1
    #base rate is 1% chance of rain
    else: 
        if (x <= 99):
            state.clear[loc] = 1
        else:
