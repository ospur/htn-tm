...
class Critic:
    ...
    def ruminate(self, w, tasks):
        ...
        if w.world.clears and w.world.clears[-1][1] > 0 and self.status == 0:
            ...
            survival0 = self.compute_exp ected_survival(hypo_mine_probabilities_0)
    ...
    def observe(self, w):
        """
        Save the current locations of the agent and boats.
        """
        ...
        for i in range(4):
            boat = "fisher" + str(i + 1)
            x, y = utils.to_coord(w.world.vessels[boat].cell)
            if x in range(20) and y in range(20):
                self.boat_traversals[i, y, x] = 1
            self.boat_directions[boat].append(w.world.vessels[boat].direction)
