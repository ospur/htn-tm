...
class MoosWorld:
    ...
    def __init__(self, state, friendly_delay, init_enemy=False, init_friendly_vessels=False, init_fisher_vessels=False):
        ...
        if init_friendly_vessels:
            self.ship_traversal = threading.Thread(target=self.ship_movement)
    ...
    def enemy_waypoint_behavior(self, waypoints, layer):
        ...
        for each in waypoints:
            ...
            flip = np.random.choice(2, 1, p=[0.55, 0.45])
            if flip == 1:
                print("\nlaying mines")
    ...
    def ship_movement(self):
        ...
        while True:
            ...
            if (self.check_experiment_completed()):
                ...
                file_object = open('results0.txt', 'a')
...
def main():
    ...
    # t = threading.Thread(target=collect_data, args=(w, critic, 5))
    # t.start()
...
def collect_data(w, critic, frequency):
    while True:
        ...
        datapoint = np.concatenate((merged, [survival]))
